...
- `最大输出 Tokens`: 单次模型回复上限。回复经常被截断时可以适当调大。
- `自动续写截断回复`: 开启后，模型因长度限制中断时会继续请求并拼接后再发送。
- `截断续写轮数`: 默认 `2`，建议 1 到 3。
- `请求超时毫秒`: 默认 `120000`。
- `最大请求次数`: 默认 `3`，包含第一次请求。填 `3` 表示最多请求 3 次。1.0.6 起使用 `maxAttempts` 配置键，旧版 `maxRetries` 不再影响请求次数。
- `重试基础间隔毫秒`: 默认 `2500`。
